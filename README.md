...
**📋 朝 8:00 JST — スケジュール取得＋選手データ先読み**

1. 競艇日和から指定選手の出走予定をスクレイピング
2. 出走情報を Discord Webhook で通知
3. 各レースの締切時刻に合わせて EventBridge Scheduler で動的スケジュールを作成
4. 選手データを先読みして DynamoDB に保存
   - 対象選手のキャリア通算成績・「当該コース進入時の全艇着順分布＋決まり手」（直近6ヶ月の条件付きデータ）
     … boatrace-db.net 由来。**同サイトは AWS のIPを遮断しているため**、`scripts/refresh_racerdb_cache.py` を
     月1回ローカルPCから実行して DynamoDB の静的キャッシュに投入しておく（Lambda はキャッシュを読む）
   - 対象選手の期間別・グレード別コース成績、直近3節の調子（競艇日和）
   - 対戦相手5人それぞれの進入コース別成績＋直近節の調子（競艇日和）

**🏁 各レース締切10分前 — AI 確率推定＋ベットエンジン（pre_race）**

1. boatrace.jp から出走表・直前情報（展示タイム・スタート展示・水面気象）・3連単オッズを取得
   - スタート展示の進入隊形を構造化パースし、前づけ等で進入が枠なりと異なる場合は
     コース別データ（条件付き分布・期間別成績・相手のコース別成績）を実コースのものに自動差し替え
   - 予想通知には進入隊形（コース順の艇番・レーサー名・展示ST）を表示
2. Bedrock Claude に選手データ一式を送り、**艇別の着順確率（1着率・2連対率・3連対率）を推定**
   （オッズは LLM に見せない — 市場に流されない独立推定）
3. Python のベットエンジンが買い目を決定:
   - Harville 型モデルで全120通りの3連単確率を展開し、市場確率（1/オッズの正規化）とブレンド
   - 期待値 `EV = p × オッズ ≥ EV_THRESHOLD` かつ確率フロア以上の買い目のみ購入
   - 総投資額はエッジスコアに連動して自動減額（上限 `RACE_BUDGET`）
   - **条件を満たす買い目がなければ購入0円で見送り**（参考買い目は通知）
4. 確率・EV・見送り理由まで DynamoDB に全記録し、Discord Webhook で通知

**📊 各レース締切20分後 — 結果収集＋収支計算（post_race）**

1. boatrace.jp の個別レース結果ページから3連単結果＋払戻金を取得
2. DynamoDB の予想データと照合して的中判定（賭け金 ÷ 100 × 払戻金 = 回収額）
3. 見送りレースは投資0円で記録し、参考買い目の答え合わせを表示
4. 日次収支を DynamoDB に記録し、Discord Webhook で結果通知を送信
5. その日の最終レースなら累計収支（見送り数含む）も更新して表示
...
- `RACE_BUDGET` (5000): 1レースあたりの上限予算（円）。エッジが薄いレースは自動減額
- `EV_THRESHOLD` (1.10): 購入する最低期待値。市場ブレンド後の値なので 1.10 ≒ モデル単体エッジ約1.45相当
- `PROB_FLOOR` (0.03): 買い目の最低確率（的中率ガード。万舟狙いを構造的に排除）
- `BLEND_LAMBDA` (0.5): モデル確率と市場確率のブレンド比
- `MAX_BETS` (5): 1レースの最大点数
- `RACER_NO` (3941 池田浩二): 対象選手。3941 の場合は選手固有プロフィールが system prompt に入る
- 舟券の種類: 3連単のみ
...
| `fetch_race_info` | boatrace.jp / kyoteibiyori.com / boatrace-db.net のページを取得して詳細データ抽出 |
